...
export function registerUserHandlers(db: PrismaClient): void {
  ipcMain.handle(IPC.AUTH_CHECK_FIRST_BOOT, async (): Promise<boolean> => (await countUsers(db)) === 0)

  ipcMain.handle(IPC.AUTH_GET_CURRENT_USER, (): AuthUser | null => {
    const s = getSession()
    return s ? { id: s.userId, fullName: s.fullName, role: s.role, lastLogin: null, createdAt: '' } : null
  })

  ipcMain.handle(IPC.AUTH_LOGIN, async (_e, { fullName, password }: { fullName: string; password: string }): Promise<LoginResult> => {
    const user = await verifyLogin(db, fullName, password)
    if (!user) return { error: 'Invalid full name or password.' }
  ...
  ipcMain.handle(IPC.USER_LIST, async () => guard(async () => {
    requireManager()
    return listUsers(db)
  }))

  ipcMain.handle(IPC.USER_CREATE, async (_e, input: { fullName: string; password: string; role: UserRole }) =>
    guard(async () => {
      requireManager()
  ...
  ipcMain.handle(IPC.USER_UPDATE, async (_e, { userId, role, password }: { userId: number; role?: UserRole; password?: string }) =>
    guard(async () => {
