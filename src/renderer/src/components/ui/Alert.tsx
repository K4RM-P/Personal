...
const VARIANT_CONFIG: Record<
  AlertVariant,
  { icon: React.ElementType; border: string; bg: string; text: string; label: string }
> = {
  success: {
    icon: CheckCircle2,
...
export function Alert({ variant, children, className }: AlertProps): React.JSX.Element {
  ...
  return (
    <div
      ...
      className={cn(
        'flex items-start gap-2 rounded-[var(--radius)] border p-3 text-xs',
        border,
        bg,
        text,
        className
    ...
    >
      <Icon
        className={cn('mt-0.5 icon-4 shrink-0', variant === 'pending' && 'animate-spin')}
        aria-hidden="true"
      />
