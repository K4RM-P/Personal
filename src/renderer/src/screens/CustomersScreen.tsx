...
export function CustomersScreen() {
  ...
  const saveCustomer = async () => {
    try {
      ...
      setNewCustomer(false); setDuplicate(null); setMessage({ type: 'success', text: 'Customer saved.' })
  ...
  const post = async () => {
    ...
    try {
      ...
      await refresh(selected.id); setAction(null); setMoney(''); setPoints(''); setNote(''); setMessage({ type: 'success', text: 'Ledger updated.' })
    } catch (error: any) { setMessage({ type: 'error', text: error.message }) }
  }
